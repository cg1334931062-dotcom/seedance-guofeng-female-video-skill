...
- If the user only gives a broad style label such as `电影感`、`游戏CG风格`、`水墨感`, the next question should first narrow what kind of `美术风格基底` or finished piece they actually mean before entering镜头层.
- Do not treat `视频风格层` as stable until `美术基底`、`成片对象`、`画面质地与情绪边界` are explicit enough to recap.
...
Typical `美术风格基底` examples for this layer:
...
If the user says only `电影感`、`游戏CG风格`、`水墨感` or another wide label, do not move on yet. Narrow what kind of美术风格基底 and成片风格 / 导演表达 they mean first, then narrow画面质地与情绪边界.
...
- After场景关系 + 光线天气 + 美术风格基底 + 成片风格 / 导演表达 + 画面质地与情绪边界 + 适配理由 are stable: `风格定型回显`
...
- with `美术基底`、`成片对象`、`画面质地与情绪边界` explicit enough to guide prompt wording rather than vibe inference
...
- After场景关系、光线天气、美术风格基底、成片风格 / 导演表达、画面质地与情绪边界 and适配理由 stabilize: `风格定型回显`
...
- with `美术基底`、`成片对象`、`画面质地与情绪边界` explicit enough to survive cross-stage reuse
