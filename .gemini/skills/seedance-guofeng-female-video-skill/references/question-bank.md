...
Make the finished piece feel like a specific kind of screen object, not just a role in a location.
...
- the user says `游戏CG风格`、`水墨感`、`动漫电影感` but has not yet narrowed the art base or finished-piece type

### Resolve these questions

- what kind of `美术风格基底` this belongs to
- what kind of `成片风格 / 导演表达` this is closest to
- what `画面质地与情绪边界` should define the image
- why that combination suits this role and scene
- whether any style conflict must be resolved before镜头层

### Good question qualities

- ask the style layer in order: `美术风格基底 -> 成片风格 / 导演表达 -> 画面质地与情绪边界`
- ask whether the piece is closer to `仙侠过场 CG`、`宣传主视觉 CG`、`国风动漫片段`、`写意水墨`、`插画动效感`、`柔光拟实拍记忆片段`
- ask whether it feels more like `剧情过场`、`角色预告 / PV`、`梦境片段`、`情绪碎片` or another specific finished-piece type
- ask whether the画面 should land more `清 / 雾`、`柔 / 压`、`克制 / 包装`
- ask how the style amplifies the role rather than just decorating her
- keep the question language director-like; if core terms are shown, only expose naturalized terms such as `美术基底` or `成片类型`
...
- treating `游戏CG风格`、`水墨感`、`动漫感` as sufficient style lock without one more美术基底 narrowing turn
- moving into镜头层 before `美术基底`、`成片对象`、`画面质地与情绪边界` are explicit
- carrying unresolved conflicts such as `留白 + 高燃` or `水墨 + 强特效游戏 CG` forward into shot design

### Minimal example

Ask whether the piece is closer to `仙侠过场 CG 的剧情片段` or `写意水墨的梦境片段` before asking how the camera should move. If the user already says `游戏CG风格`, first ask what kind of CG finished piece it should be, not how many shots it has.
