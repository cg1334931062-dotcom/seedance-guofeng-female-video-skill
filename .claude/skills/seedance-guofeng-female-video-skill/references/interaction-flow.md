...
- In Claude Code plan mode, option presentation is mandatory for every question step, even if the user is not stuck and has not explicitly requested options.
- In normal mode, continue to only show options when the user is stuck or explicitly asks for them.
...
- Treat the template library as hidden support. Only surface template-like options when the user is clearly stuck, explicitly wants option sets, or you are running in Claude Code plan mode.
- When running in Claude Code plan mode, always surface 2-5 context-specific, dynamically generated options at each interaction step to support the plan mode interactive selection mechanism.
