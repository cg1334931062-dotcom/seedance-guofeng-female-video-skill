...
默认情况下，skill 的最终交付会直接在当前对话中返回，不会自动写入工作区文件；只有用户明确要求落盘时才应创建文件。
...
`视频风格` 不再要求用户显式共创一整套风格字段，而是先像导演沟通一样把成片锁住。
...
内部仍然会把这些回答映射到隐藏中间层，确保最后的 prompt 仍然可执行。

也就是说，像 `游戏CG风格`、`电影感`、`水墨感` 这样的回答，不会被直接视为已锁定风格，而是会先被拆成“像哪类片”和“画面更怎么呈现”。
...
- 默认直接在对话中返回最终提示词包和质检建议
- 不自动创建成品 `.md`、`.txt` 或其他结果文件
- 只有用户明确要求“写入文件 / 落盘 / 生成示例文件”时才输出到文件
